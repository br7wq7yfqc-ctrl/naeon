...
|----|------|------|-----|-------|
| **ST-A** | Strategy-камера + **один** модуль на unnamed паде | **built:** `StrategyOverlay` (B) / `BaseBuilder.place_player_habitat`; **FL-A/B/C/D/E/F/G** fleet pips (cap 8; `FLEET n/8`) | overlay на Nex-Prime; habitat на `Pad_North` / `Pad_Approach` / `Pad_Flank`; корабль и TPS живы; семь allied SoftNet/visitor pips; click ≠ combat | вторая галактика; mint `SITE_*`; 10–15 ships |
