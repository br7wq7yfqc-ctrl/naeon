...
|----|------|------|-----|-------|
| **ST-A** | Strategy-камера + **один** модуль на unnamed паде | **built:** `StrategyOverlay` (B) / `BaseBuilder.place_player_habitat`; **FL-A/B/C/D/E/F/G/H/I/J/K/L/M** fleet pips (cap 14; `FLEET n/14`) | overlay на Nex-Prime; habitat на `Pad_North` / `Pad_Approach` / `Pad_Flank`; корабль и TPS живы; тринадцать allied SoftNet/visitor pips; click ≠ combat | вторая галактика; mint `SITE_*`; 10–15 ships |
