...
|----|------|------|-----|-------|
| **ST-A** | Strategy-камера + **один** модуль на unnamed паде | **built:** `StrategyOverlay` (B) / `BaseBuilder.place_player_habitat`; **FL-A/B/C/D/E/F** fleet pips (cap 7; `FLEET n/7`) | overlay на Nex-Prime; habitat на `Pad_North` / `Pad_Approach` / `Pad_Flank`; корабль и TPS живы; шесть allied SoftNet/visitor pips; click ≠ combat | вторая галактика; mint `SITE_*`; 10–15 ships |
