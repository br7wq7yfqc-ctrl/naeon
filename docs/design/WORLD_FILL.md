...
| 2 | Locked plates without honest GLB — occupy pylon **built** | Beacons: `ClaimBeaconVis` tagged `ledger_slug=cybernex_claim_beacon` / `grot_claim_beacon` (dump `phjM0` / `nFxgT`); empty `site_pin`; `uuid=""`. GLB path first, else code-first pylon (headless = meta-only). Pad / extractor / debris — **slug в ledger, lock-строки в `approved_sketches` нет** — сначала lock существующей пластины, не mint ID. Outpost-kit: slug нет → skip | Tripo; выдумать UUID; SITE_*; вторая волна капиталок; новая slice letter |
