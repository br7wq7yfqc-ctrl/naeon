...
system. **Version 1.5, 2026-08-28.** WF-A density slice built (unnamed pads).
...
| # | Что | Зачем | Не делать |
|---|-----|-------|-----------|
| 1 | Ready-made fill | OS-D/E: unnamed dirt/rock/tile/crate/cable/HDRI в `s3://neon`. OS-D denser: code-first scatter + pad clutter с **уже существующих** slug (`debris_cluster`, `t1_resource_extractor`, `utility_bay`) и filler ID (`pad_crate_cc0`, `outpost_mast_cc0`). OS-G: одна мачта/habitat на уже существующем паде. **WF-A built:** один density-срез на тех же unnamed Nex-Prime падах (rim crate/debris; `pad_fill=22`; slug shelf only; `new_SITE_*=0`; `new_UUID=0`). OS-G силуэт не легенда. | Tripo на грязь; commit в git; mint SITE_*; новый slug / UUID |
| 2 | Locked plates honest GLB — occupy pylon **built + lod-budget** | Beacons: `ClaimBeaconVis` tagged `ledger_slug=cybernex_claim_beacon` / `grot_claim_beacon` (dump `phjM0` / `nFxgT`); empty `site_pin`; `uuid=""`. GUI loads `props/{slug}/{slug}_lod1.glb` (image_to_model, keep PBR, faction-only). `process_asset` exports the LOD object only (was base+lod → 88k/93k verts); `--stand-up` (CX X→Z); `--max-verts 12000,4000,1500`; `--max-tex 1024`. lod1 ≈ 3.9k/3.7k verts, 336/434 KB. Generic `props/claim_beacon/` is fallback only. Headless = meta-only. Pad / extractor / debris — **slug в ledger, lock-строки в `approved_sketches` нет** — сначала lock существующей пластины, не mint ID. Outpost-kit: slug нет → skip | Tripo dirt; выдумать UUID; SITE_*; вторая волна капиталок; новая slice letter |
