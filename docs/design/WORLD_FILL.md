...
| 2 | Locked plates honest GLB — occupy pylon **built**; slug-folder GLB from locked dumps **this pass** | Beacons: `ClaimBeaconVis` tagged `ledger_slug=cybernex_claim_beacon` / `grot_claim_beacon` (dump `phjM0` / `nFxgT`); empty `site_pin`; `uuid=""`. GUI loads `props/{slug}/{slug}_lod1.glb` (image_to_model on those plates, keep PBR, faction-only). Generic `props/claim_beacon/` is fallback only, not those dumps. Headless = meta-only. Pad / extractor / debris — **slug в ledger, lock-строки в `approved_sketches` нет** — сначала lock существующей пластины, не mint ID. Outpost-kit: slug нет → skip | Tripo dirt; выдумать UUID; SITE_*; вторая волна капиталок; новая slice letter |
