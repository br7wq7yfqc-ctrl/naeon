...
| 3 | Ship as place (interior, seats, multi-crew) | `InteriorDirector` / `InteriorGenerator`; Seat; F seat→pilot; MC-A `CrewSeat`; MC-B gunner label; MC-C engineer seat; MC-D ops seat; SN-B hull SoftNet | seat↔pocket↔hatch EVA; двери ведут в карман/EVA; **MC-A** first crew seat (F/I same pocket); **MC-B** `GUNNER` label; **MC-C** `ENGINEER` third seat; **MC-D** `OPS` fourth seat, HUD CREW n/4; **SN-B** second local viewer on seated hull sees SoftNet visual hull/pilot puppet (host Pulse/occupy/thrust; no second hull) | Phase 2 multi-crew 2–4 complete | фейковые двери |
...
| 10 | Industry | RBE / biomass / Contribution / `Extractor` / `ResourceNode` / pad harvest | ST-A habitat; ST-B extractor + HUD Contribution; ST-C print one catalog module at pad bench; ST-D hangar queue on catalog carrier; ST-E player orbital cluster (dock + habitat); ST-F CX↔GR owner swap (theme + services, same-tier numbers); **ST-J** pad hangar stub; **BR-A** Biomass Rank 0–4 HUD label from lifetime wallet; **CR-A** Contribution Rank 0–4 HUD label from lifetime wallet | — | P2W richer nodes |
