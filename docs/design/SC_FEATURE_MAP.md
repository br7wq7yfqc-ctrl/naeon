...
| 3 | Ship as place (interior, seats, multi-crew) | `InteriorDirector` / `InteriorGenerator`; Seat; F seat→pilot; MC-A `CrewSeat`; MC-B gunner label | seat↔pocket↔hatch EVA; двери ведут в карман/EVA; **MC-A** first crew seat (F/I same pocket); **MC-B** `GUNNER` label | Phase 2 more seats / roles | фейковые двери |
...
| 10 | Industry | RBE / biomass / Contribution / `Extractor` / `ResourceNode` / pad harvest | ST-A habitat; ST-B extractor + HUD Contribution; ST-C print one catalog module at pad bench; ST-D hangar queue on catalog carrier; ST-E player orbital cluster (dock + habitat); ST-F CX↔GR owner swap (theme + services, same-tier numbers); **ST-J** pad hangar stub | — | P2W richer nodes |
...
| 13 | Traffic / AI | dummy / pad guard / Clash bots | NPC can fly the existing loop (NP-A); occupy/harvest on unnamed pad (NP-B); player invites one NPC into the squad (NP-D); short offline pad/follow cycle (NP-F); two NPCs soft-alliance / raid or logistics intent (NP-E); **BT-A** pad-guard 3-state BT (patrol / engage Pulse / return); **BT-B** visitor 3-state BT (approach / hold / leave) | Clash density later | generated galaxy traffic |
