...
| 10 | Industry | RBE / biomass / Contribution / `Extractor` / `ResourceNode` / pad harvest | ST-A habitat; ST-B extractor + HUD Contribution; ST-C print one catalog module at pad bench; ST-D hangar queue on catalog carrier; ST-E player orbital cluster (dock + habitat); ST-F CX↔GR owner swap (theme + services, same-tier numbers); **ST-J** pad hangar stub; **BR-A** Biomass Rank 0–4 HUD label from lifetime wallet | — | P2W richer nodes |
