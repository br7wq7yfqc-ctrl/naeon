...
| 9 | Contracts / loops | SessionObjectives; `ContractBoard` | Q-A one pad template; Q-B one alliance-shared occupy/logistics; Q-C optional Learning Node; Q-D NPC offers same Q-A board id | campaigns later | story=power |
| 10 | Industry | RBE / biomass / Contribution / `Extractor` / `ResourceNode` / pad harvest | ST-A habitat; ST-B extractor + HUD Contribution; ST-C print one catalog module at pad bench; ST-D hangar queue on catalog carrier; ST-E player orbital cluster (dock + habitat); ST-F CX↔GR owner swap (theme + services, same-tier numbers) | — | P2W richer nodes |
