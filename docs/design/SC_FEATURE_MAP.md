...
| 10 | Industry | RBE / biomass / Contribution / `Extractor` / `ResourceNode` / pad harvest | ST-A habitat; ST-B extractor + HUD Contribution; ST-C print one catalog module at pad bench; ST-D hangar queue on catalog carrier; ST-E player orbital cluster (dock + habitat); ST-F CX↔GR owner swap (theme + services, same-tier numbers); **DO-A** contested CX↔GR transition on one occupied unnamed pad (SoftKnowledge CONTESTED / CYBERNEX / GROT); **DO-B** same contested grammar on the existing `PlayerOrbitalStation` cluster (ST-E/ST-G stay); **ST-J** pad hangar stub; **ST-K** orbital hangar stub on that same cluster (ST-E/ST-G/ST-J stay); **BR-A** Biomass Rank 0–4 HUD label from lifetime wallet; **CR-A** Contribution Rank 0–4 HUD label from lifetime wallet | — | P2W richer nodes |
