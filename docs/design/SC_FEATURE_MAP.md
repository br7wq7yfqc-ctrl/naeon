...
| 3 | Ship as place (interior, seats, multi-crew) | `InteriorDirector` / `InteriorGenerator`; Seat; F seat→pilot; MC-A `CrewSeat`; MC-B gunner label; MC-C engineer seat; SN-B hull SoftNet | seat↔pocket↔hatch EVA; двери ведут в карман/EVA; **MC-A** first crew seat (F/I same pocket); **MC-B** `GUNNER` label; **MC-C** `ENGINEER` third seat, HUD CREW n/3; **SN-B** second local viewer on seated hull sees SoftNet visual hull/pilot puppet (host Pulse/occupy/thrust; no second hull) | Phase 2 more seats / roles | фейковые двери |
