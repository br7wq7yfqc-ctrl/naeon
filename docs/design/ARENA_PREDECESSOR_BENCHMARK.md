...
| 2 | Топология: 3 линии, jungle, river, вертикаль / jump pads | `ClashLanes` TOP/MID/BOT на полу ~60×60 | **3 линии** + **AR-D** camp + **AR-J** prime camp + **река** (`ClashRiver`) + **jump pads** (`ClashJumpPads`, короткий hop) на том же 60×60 | feel lock; не city-map | новая city-map; SITE_* mint |
...
| 6 | Heroes / kits | `AbilityKitCatalog` + `AbilitySystem`; формы = идентичность | **AR-E + AR-L + AR-M + AR-N + AR-O + AR-P + AR-Q:** 10 китов × 4 слота (CX Nex/Grid/Lattice/Prism/Helix, GR Rot/Spore/Vein/Facet/Coil; Pulse / utility / probe\|surge / Form Cycle) | feel lock; формы не rank-стат; toward 6–8 | P2W kits; форма = скрытый MMR |
...
| `AbilityKitCatalog.gd` | 10 китов (AR-E 4 + AR-L Lattice + AR-M Vein + AR-N Prism + AR-O Facet + AR-P Helix + AR-Q Coil); costs из `EnergyEconomy`; `kit_for_faction` = прежний default |
...
| Phase 3 план | 6–8 heroes / items / matchmaking — **ещё не код**; jungle bite = AR-D + AR-J; items-shop seed = AR-K; fifth kit = AR-L; sixth kit = AR-M; seventh kit = AR-N; eighth kit = AR-O; ninth kit = AR-P; tenth kit = AR-Q (5v5 local = AR-G) |
...
| **AR-Q** | tenth AbilityKit toward 6–8 | **сделано:** один extra кит GR Coil на том же `AbilityKitCatalog` (Pulse / Coil Seal / Coil Probe / Form Cycle); SoftKnowledge COIL; prior 9 kits stay; AR-K bench stays | этот срез | P2W kits; форма=стат; leftover 5v5 soak |
...
AR-Q: десятый Clash AbilityKit (GR Coil) на существующем `AbilityKitCatalog` / TestArena / ClashDirector — gROT-пара к CX Helix. Та же грамматика Pulse / utility / probe|surge / Form Cycle. SoftKnowledge / HUD `COIL` only. Prior 9 kits stay. ClashModuleBench AR-E SENSOR + AR-K CARGO stay. AR-A…AR-P, река и jump pads не откатывать. G5 закрыт. Не mint SITE_*. Knowledge не меняет DPS. Дверь: меню → AEXION CLASH → TestArena.
