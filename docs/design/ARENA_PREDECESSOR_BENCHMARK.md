...
| `ClashMatchDirector.gd` | K/D, banner, lane HUD; **AR-T** SoftKnowledge `WAVE` / `MINION`; **AR-U** SoftKnowledge `XP` / `LEVEL`; **AR-X** SoftKnowledge `CAMP` / `JUNGLE`; no P2W |
...
| `TestArena.gd` | дверь слоя; beacons Neutral; `ClashWaves` на полосах (**AR-T** WAVE/MINION + **AR-V** opposite + **AR-W** MID); `ClashCamp` + `ClashPrimeCamp` + `ClashSmallCamp`; один `ClashModuleBench` (AR-E SENSOR + AR-K CARGO); `ClashRiver`; `ClashJumpPads`; `ClashLocalMatch` 5v5 (3v3 startable); **AR-U** XP/LEVEL HUD |
...
AR-X: one extra small off-lane `ClashCamp` on the existing TestArena / ClashDirector 60×60. Weaker than AR-J prime. AR-D fangtooth stays. Drop = soft War Score only. SoftKnowledge / HUD `CAMP` / `JUNGLE` only. Host authority. Pulse 11. Infection cap 5. Not a 4th minion lane. Not a 13th kit. Not another fleet pip. AbilityKitCatalog stays at 12 (`cx_nex`…`gr_thorn`). FL-N FLEET 15/15 stays. AR-T / AR-V / AR-W waves stay. AR-U XP/LEVEL stay informational. AR-A…AR-W, река и jump pads не откатывать. G5 закрыт. Не mint SITE_*. Knowledge не меняет DPS. Дверь: меню → AEXION CLASH → TestArena.
