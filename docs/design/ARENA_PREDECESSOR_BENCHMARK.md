...
| 6 | Heroes / kits | `AbilityKitCatalog` + `AbilitySystem`; формы = идентичность | **AR-E + AR-L + AR-M + AR-N + AR-O:** 8 китов × 4 слота (CX Nex/Grid/Lattice/Prism, GR Rot/Spore/Vein/Facet; Pulse / utility / probe\|surge / Form Cycle) | feel lock; формы не rank-стат; toward 6–8 | P2W kits; форма = скрытый MMR |
...
| `AbilityKitCatalog.gd` | 8 китов (AR-E 4 + AR-L Lattice + AR-M Vein + AR-N Prism + AR-O Facet); costs из `EnergyEconomy`; `kit_for_faction` = прежний default |
...
| Phase 3 план | 6–8 heroes / items / matchmaking — **ещё не код**; jungle bite = AR-D + AR-J; items-shop seed = AR-K; fifth kit = AR-L; sixth kit = AR-M; seventh kit = AR-N; eighth kit = AR-O (5v5 local = AR-G) |
...
AR-O: восьмой Clash AbilityKit (GR Facet) на существующем `AbilityKitCatalog` / TestArena / ClashDirector — gROT-слот после CX Prism / GR Vein. Та же грамматика Pulse / utility / probe|surge / Form Cycle. SoftKnowledge / HUD `FACET` only. Prior 7 kits stay. ClashModuleBench AR-E SENSOR + AR-K CARGO stay. AR-A…AR-N, река и jump pads не откатывать. G5 закрыт. Не mint SITE_*. Knowledge не меняет DPS. Дверь: меню → AEXION CLASH → TestArena.
