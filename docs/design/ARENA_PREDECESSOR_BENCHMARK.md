...
| Phase 3 план | 6–8 heroes / items / matchmaking — **ещё не код**; jungle bite = AR-D + AR-J; items-shop seed = AR-K (5v5 local = AR-G) |
...
AR-K: вторая session catalog option на существующем `ClashModuleBench` (AR-E SENSOR stays; CARGO / Nex Hold). SoftKnowledge / HUD `HOLD` only. Не колода Paragon, не cash-shop, не unique weapon. AbilityKitCatalog 4 kits stay. AR-A…AR-J, река и jump pads не откатывать. G5 закрыт. Не mint SITE_*. Knowledge не меняет DPS. Дверь: меню → AEXION CLASH → TestArena.
