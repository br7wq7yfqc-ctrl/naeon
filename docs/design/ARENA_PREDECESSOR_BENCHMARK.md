...
| 6 | Heroes / kits | `AbilityKitCatalog` + `AbilitySystem`; формы = идентичность | **AR-E + AR-L + AR-M + AR-N:** 7 китов × 4 слота (CX Nex/Grid/Lattice/Prism, GR Rot/Spore/Vein; Pulse / utility / probe\|surge / Form Cycle) | feel lock; формы не rank-стат; toward 6–8 | P2W kits; форма = скрытый MMR |
...
| `AbilityKitCatalog.gd` | 7 китов (AR-E 4 + AR-L Lattice + AR-M Vein + AR-N Prism); costs из `EnergyEconomy`; `kit_for_faction` = прежний default |
...
| Phase 3 план | 6–8 heroes / items / matchmaking — **ещё не код**; jungle bite = AR-D + AR-J; items-shop seed = AR-K; fifth kit = AR-L; sixth kit = AR-M; seventh kit = AR-N (5v5 local = AR-G) |
...
AR-N: седьмой Clash AbilityKit (CX Prism) на существующем `AbilityKitCatalog` / TestArena / ClashDirector — Cybernex-слот после CX Lattice / GR Vein. Та же грамматика Pulse / utility / probe|surge / Form Cycle. SoftKnowledge / HUD `PRISM` only. Prior 6 kits stay. ClashModuleBench AR-E SENSOR + AR-K CARGO stay. AR-A…AR-M, река и jump pads не откатывать. G5 закрыт. Не mint SITE_*. Knowledge не меняет DPS. Дверь: меню → AEXION CLASH → TestArena.
