...
| 5 | Waves / minions | `CombatDummy` с `lane_spawn_table()` | **AR-C + AR-T:** `ClashWaves` — периодические волны `CombatDummy` по линии; **AR-T seed** host-authority Pulse 11; SoftKnowledge `WAVE` / `MINION`; march к OUTER; без магазина | feel lock | магазин-миньоны |
