...
| `TestArena.gd` | дверь слоя; beacons Neutral; `ClashWaves` на полосах; `ClashCamp` + `ClashPrimeCamp`; один `ClashModuleBench`; `ClashRiver`; `ClashJumpPads`; `ClashLocalMatch` 5v5 (3v3 startable) |
...
| Phase 3 план | 6–8 heroes / items / matchmaking — **ещё не код**; jungle bite = AR-D + AR-J (5v5 local = AR-G) |
...
AR-J: один extra off-lane prime-class `ClashCamp` на том же 60×60 TestArena (AR-D fangtooth stays). Soft contest; drop = soft WS (rules/13), не unique weapon. SoftKnowledge / HUD labels only. AR-A…AR-I, река и jump pads не откатывать. G5 закрыт. Не mint SITE_*. Knowledge не меняет DPS. Дверь: меню → AEXION CLASH → TestArena.
