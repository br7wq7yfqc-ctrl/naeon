...
| `BaseBuilder.gd` | P0 controller-only; ST-A `place_player_habitat`; NP-C `place_npc_habitat`; ST-C/NP-G `print_catalog_module`; ST-G/NP-I `print_factory_catalog_module`; ST-H `place_pad_turret`; ST-I `place_pad_storage`; ST-J `place_pad_hangar_stub`; ST-K `place_orbital_hangar_stub`; ST-L `place_orbital_turret`; ST-M `place_orbital_storage` | mint SITE_* |
...
| `PlayerOrbitalStation.gd` | ST-E dock+habitat; ST-G factory; ST-K hangar stub; ST-L defense turret; ST-M storage; **NP-I** same bench (c) | mint SITE_*; city |
