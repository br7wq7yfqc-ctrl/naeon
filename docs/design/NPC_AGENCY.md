...
**Дата:** 2026-08-18  
**Движок:** Godot 4.7.2  
**Статус:** NP-A…NP-F + NP-C + NP-G built. **NP-H this pass** (ST-D hangar queue of one catalog module).  
**Код срезов:** NP-A flight · NP-B occupy/harvest · NP-C habitat · NP-D invite · NP-E soft alliance · NP-F offline · NP-G print · **NP-H hangar**. Не G2–G6. Не 10k CCU.
...
|------|-----------|--------|
| `PadTraffic.gd` | 1 pad-guard `CombatDummy` + visitor `ShipController`/`NpcPilot` (NP-A flight, NP-B occupy/harvest, NP-C habitat, NP-G ST-C print, NP-H ST-D hangar, NP-D squad invite, NP-F short offline pad/follow, NP-E soft alliance / raid-or-logistics intent) + surface dummy под Pulse | factory (c); mobile SITE_* |
...
Каждый срез playable сам, **local-first** (offline и coop SoftNet visual). NP-A…F + NP-C + NP-G + **NP-H** built.
...
Порядок: A → B → D → F → C (habitat) → E → G (ST-C print) → **H (ST-D hangar)**. Не открывать G2 «чтобы было больше миров NPC».
...
| Сейчас | Дальше |
|--------|--------|
| NP-A…F + NP-C habitat + NP-G ST-C print + **NP-H** ST-D hangar (one catalog module) | следующий NP-срез: factory (c) |
| Стратегия — ST-A…ST-G built; NPC print = bench (a); NPC hangar = (b) | factory (c) не этот срез |
