...
17. Next: 10-min soak on Mac GPU for the FPS half of rules/25 — harness
    `--playtest-soak` / `scripts/playtest_mac_gpu_soak.sh` (GUI Godot 4.7.2,
    min preset, honest TIME_FPS). This VM is llvmpipe. Record FPS/RAM in
    `docs/PROTOTYPE_TO_PLAYABLE.md` after the Mac run.
...
53. FL-L: twelfth extra allied pip on ST-A Strategy overlay (key B).
    ...
    Infection cap 5. ORBITAL_STATIONS stays false. No SITE_*
    (this pass).
54. AR-R: eleventh Clash AbilityKit (CX Spire) on AbilityKitCatalog
    (TestArena / ClashDirector 60×60) toward Phase-3 6–8 heroes.
    Cybernex symmetric slot after GR Coil / CX Helix. Same Pulse /
    utility / probe|surge / Form Cycle grammar from EnergyEconomy.
    SoftKnowledge / HUD SPIRE / NEX SPIRE only. Knowledge ≠ DPS /
    yield / Pulse / Hack / rank. No cash-shop, P2W kits, or unique
    weapon. No Paragon card deck. Prior 10 kits stay. ClashModuleBench
    AR-E SENSOR + AR-K CARGO stay. AR-A…AR-Q stay. Infection cap 5.
    Host authority. ORBITAL_STATIONS stays false. No SITE_* (this pass).
55. FL-M: thirteenth extra allied pip on ST-A Strategy overlay (key B).
    SoftNet visual hull (pad-visitor / NP-A grammar). Cap 14
    (player + 13) toward 10–15. HUD FLEET n/14 SoftKnowledge only.
    Click/select ≠ combat. Host Pulse / occupy. FL-A…FL-L stay.
    Not 10–15 ships. Not a second OpenSpace. Not ENet. Not G5.
    Infection cap 5. ORBITAL_STATIONS stays false. No SITE_*
    (this pass).
56. AR-S: twelfth Clash AbilityKit (GR Thorn) on AbilityKitCatalog
    ...
    authority. ORBITAL_STATIONS stays false. No SITE_* (this pass).
57. FL-N: fourteenth extra allied pip on ST-A Strategy overlay (key B).
    SoftNet visual hull (pad-visitor / NP-A grammar). Cap 15
    (player + 14) closes the 10–15 bar as SoftNet pips. HUD FLEET n/15
    SoftKnowledge only. Click/select ≠ combat. Host Pulse / occupy.
