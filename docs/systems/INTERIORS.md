...
**Status:** IN-A + IN-B + IN-C V0/V1 + MC-A crew seat + MC-B gunner label built · Procedural seed · **Cost:** 0 Tripo
...
4. **F** at the ship `Seat` → pilot. **F** at ship `CrewSeat` (MC-A) boards crew; **I** returns to the same `ship_int` pocket. The crew seat role is `gunner` (MC-B SoftKnowledge / HUD label only). **F** at `OpsSeat` (station) or `HangarSeat` (hangar) boards that legal seat. **I** from the legal seat returns to the same pocket (no exterior hop). Landed **F** from the ship is still the pad-deck walker.
...
- Ship crew seat is `CrewSeat` / `CrewSeatVolume` (MC-A). Same IN-B F→seat I→pocket gating. Station role `gunner` is a label (MC-B). Not a second hull.
- FloatingOrigin still tracks player

## Future
- Swap rooms for HQ dual-theme GLB modules when budget allows
- Multi-crew ship stations (MC-A seat + MC-B gunner label built)
...
Updated: 2026-09-01 — MC-B gunner role label on the MC-A crew seat. IN-A/IN-B/IN-C unchanged.
