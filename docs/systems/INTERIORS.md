...
**Status:** IN-A + IN-B + IN-C V0/V1 + MC-A crew seat + MC-B gunner label + MC-C engineer seat + MC-D ops seat built · Procedural seed · **Cost:** 0 Tripo
...
4. **F** at the ship `Seat` → pilot. **F** at ship `CrewSeat` (MC-A) boards crew; **I** returns to the same `ship_int` pocket. The crew seat role is `gunner` (MC-B SoftKnowledge / HUD label only). **F** at `EngineerSeat` (MC-C) boards crew with `engineer` label. **F** at `OpsCrewSeat` (MC-D) boards crew with `ops` label; HUD `CREW n/4`. **F** at `OpsSeat` (station) or `HangarSeat` (hangar) boards that legal seat. **I** from the legal seat returns to the same pocket (no exterior hop). Landed **F** from the ship is still the pad-deck walker.
...
- Ship crew seat is `CrewSeat` / `CrewSeatVolume` (MC-A). Same IN-B F→seat I→pocket gating. Station role `gunner` is a label (MC-B). Third seat `EngineerSeat` (MC-C) is a label. Fourth seat `OpsCrewSeat` (MC-D) is a label. Not a second hull. Not station `OpsSeat`.
- FloatingOrigin still tracks player

## Future
- Swap rooms for HQ dual-theme GLB modules when budget allows
- Multi-crew ship stations (MC-A…MC-D built — gunner / engineer / ops labels)
...
Updated: 2026-09-02 — MC-D ops crew seat on the player hull pocket. IN-A/IN-B/IN-C unchanged.
