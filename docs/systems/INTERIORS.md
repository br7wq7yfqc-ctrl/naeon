...
**Status:** IN-A + IN-B built · Procedural seed · **Cost:** 0 Tripo
...
7. **I** at airlock/hatch → EVA (zero-G if not landed) or pad. Doors open into the next room or the hatch. Station/hangar hatch returns to pad or dock in the same `OpenSpace.tscn` (not MainMenu).
...
Updated: 2026-08-28 — IN-B ops console + legal seats + life-support (headless PASS). IN-A pockets unchanged.
