...
4. **F** at the ship `Seat` → pilot. **F** at ship `CrewSeat` (MC-A) boards crew; **I** returns to the same `ship_int` pocket. The crew seat role is `gunner` (MC-B SoftKnowledge / HUD label only). **F** at `EngineerSeat` (MC-C) boards crew with `engineer` label; HUD `CREW n/3`. **F** at `OpsSeat` (station) or `HangarSeat` (hangar) boards that legal seat. **I** from the legal seat returns to the same pocket (no exterior hop). Landed **F** from the ship is still the pad-deck walker.
...
- Ship crew seat is `CrewSeat` / `CrewSeatVolume` (MC-A). Same IN-B F→seat I→pocket gating. Station role `gunner` is a label (MC-B). Third seat `EngineerSeat` (MC-C) is a label. Not a second hull.
