...
**Status:** IN-A + IN-B + IN-C V0/V1 + MC-A crew seat built · Procedural seed · **Cost:** 0 Tripo
...
4. **F** at the ship `Seat` → pilot. **F** at ship `CrewSeat` (MC-A) boards crew; **I** returns to the same `ship_int` pocket. **F** at `OpsSeat` (station) or `HangarSeat` (hangar) boards that legal seat. **I** from the legal seat returns to the same pocket (no exterior hop). Landed **F** from the ship is still the pad-deck walker.
...
- Ship crew seat is `CrewSeat` / `CrewSeatVolume` (MC-A). Same IN-B F→seat I→pocket gating. Not a second hull.
- FloatingOrigin still tracks player

## Future
- Swap rooms for HQ dual-theme GLB modules when budget allows
- Multi-crew ship stations (MC-A: first crew seat built)
...
Updated: 2026-09-01 — MC-A first crew seat on the player ship pocket. IN-A/IN-B/IN-C unchanged.
