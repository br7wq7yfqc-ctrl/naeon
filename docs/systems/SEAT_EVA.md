...
## Crew seat [F] (MC-A + MC-B + MC-C + MC-D)
...
- HUD `CREW n/4` is a label only. Knowledge does not change thrust / DPS / yield.
...
- **MC-C:** third legal seat `EngineerSeat` (`engineer_seat` offset). Station role `engineer` is a SoftKnowledge / HUD label (`ENGINEER`). Gunner seat stays. F/I role stays `crew`. Pilot F/I unchanged.
- **MC-D:** fourth legal seat `OpsCrewSeat` (`ops_seat` offset). Station role `ops` is a SoftKnowledge / HUD label (`OPS`). HUD `CREW n/4`. Engineer seat stays. F/I role stays `crew`. Pilot F/I unchanged.
