...
- Lane waves (AR-C + AR-T + AR-V + AR-W): `ClashWaves` timed `CombatDummy` proxies march TOP/MID/BOT toward enemy OUTER; **AR-T seed** is host-authority Pulse 11 with SoftKnowledge / HUD `WAVE` / `MINION` only; **AR-V** mirrors that seed on the opposite Clash lane; **AR-W** is the remaining Clash lane (MID); hit/be-hit via existing Turret hooks; no shop / P2W / XP-as-power / 13th kit / new GLB
...
- Third-lane wave (AR-W): remaining Clash lane (MID) gets the same host-authority `CombatDummy`, Pulse 11, SoftKnowledge `WAVE` / `MINION`. Completes the 3-lane MOBA seed. AR-T and AR-V stay. Not a 13th kit. Not XP power. Not another fleet pip. Catalog stays at 12. FL-N FLEET 15/15 stays. AR-U XP/LEVEL stay informational.
...
- Rewards pipeline seed (AR-Y): SoftKnowledge / HUD `REWARD` / `TITLE` on match-end (rules/13 cosmetic / title / lore). Grant is informational — never unique combat item / Pulse / yield / kit unlock. Infection cap 5. Catalog stays at 12. FL-N FLEET 15/15 stays. AR-X small jungle camp stays. AR-W third-lane wave stays. AR-V second-lane wave stays. AR-U XP/LEVEL stay informational.
- Matchmaking seed (AR-Z): SoftKnowledge / HUD `MATCH` / `QUEUE` / `READY`. Local host-authority queue/ready before 3v3/5v5 on the same 60×60. Informational only — never pay-rank matchmaking, P2W queue skip, unique combat item / Pulse / yield / kit unlock. Infection cap 5. Catalog stays at 12. FL-N FLEET 15/15 stays. AR-Y REWARD/TITLE stays.
