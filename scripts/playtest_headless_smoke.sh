...
if grep -q '\[Playtest\] FAIL' /tmp/pt_mech.log; then
  MECH_CODE=1
elif grep -q '\[Playtest\] FL-L PASS' /tmp/pt_mech.log && grep -q 'FLEET MANIFEST 13/13' /tmp/pt_mech.log && grep -q '\[Playtest\] PASS AR-P' /tmp/pt_mech.log && grep -q 'ninth=cx_helix' /tmp/pt_mech.log; then
  MECH_CODE=0
else
  MECH_CODE=1
fi
echo MECH_CODE=$MECH_CODE MECH_ERR=$(grep -c 'SCRIPT ERROR' /tmp/pt_mech.log || true)
grep -E 'Playtest|SCRIPT ERROR|OS-H|ST-A|FL-L|FLEET MANIFEST 13/13|PASS AR-P|ninth=cx_helix|kits=' /tmp/pt_mech.log | tail -80 || true
