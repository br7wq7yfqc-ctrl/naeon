...
AR_CODE=$?
set -e
if grep -q '\[Playtest\] PASS arena AR-A' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-B' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-C' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-D' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-E' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-F' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-G' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-I' /tmp/pt_ar.log && grep -q '\[Playtest\] PASS arena AR-J' /tmp/pt_ar.log && grep -q '\[Playtest\] river present on footprint' /tmp/pt_ar.log && grep -q '\[Playtest\] jump pads hop on footprint' /tmp/pt_ar.log; then
  AR_CODE=0
elif grep -q '\[Playtest\] FAIL arena AR-A' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-B' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-C' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-D' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-E' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-F' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-G' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-I' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL arena AR-J' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL river' /tmp/pt_ar.log || grep -q '\[Playtest\] FAIL jump pads' /tmp/pt_ar.log; then
  AR_CODE=1
elif [ "$AR_CODE" -eq 0 ]; then
  AR_CODE=1
fi
echo AR_CODE=$AR_CODE AR_ERR=$(grep -c 'SCRIPT ERROR' /tmp/pt_ar.log || true)
grep -E 'Playtest|SCRIPT ERROR|AR-A|AR-B|AR-C|AR-D|AR-E|AR-F|AR-G|AR-I|AR-J|river|jump pad|ClashJumpPads|prime' /tmp/pt_ar.log | head -80 || true
