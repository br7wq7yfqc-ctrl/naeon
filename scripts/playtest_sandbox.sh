...
echo "--- OpenSpace boot ---"
grep -E 'SCRIPT ERROR|ERROR|OpenSpace|PlanetBody|Surface|Landscape|Cave|seed' /tmp/sb_os.log | head -40 || true
echo "--- TestArena ---"
grep -E 'SCRIPT ERROR|ERROR|TestArena' /tmp/sb_ta.log | head -30 || true
echo "--- Sandbox probe ---"
grep -E '\[Sandbox\]|SCRIPT ERROR|ERROR' /tmp/sb_os_probe.log | head -80 || true
echo "--- Mechanics (not a human gate) ---"
grep -E '\[Playtest\]|SCRIPT ERROR' /tmp/sb_mech.log | head -20 || true

if grep -q 'HUMAN_UNFIT' /tmp/sb_os_probe.log 2>/dev/null; then
  echo "[sandbox] probe verdict HUMAN_UNFIT (expected)"
else
  echo "[sandbox] probe did not print HUMAN_UNFIT — treat as incomplete run"
fi
echo "[sandbox] HUMAN_UNFIT — do not stamp PASS"
exit 0
