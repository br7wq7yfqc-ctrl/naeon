...
## AR-L / AR-M / AR-N / AR-O / AR-P / AR-Q / AR-R / AR-S: name a Clash AbilityKit. Never DPS / yield / Pulse / Hack / rank.
static func kit_label(kit_id: String = "") -> String:
	...
	match str(kit_id):
		...
		"gr_thorn":
			return "ROT THORN" if named else "THORN"
		_:
...
## PV-C: name Strategy overlay PvP. Never Pulse / HP / kit unlock.
static func pvp_label() -> String:
	var hist := mastery("history")
	var combat := mastery("combat")
	if hist >= 10.0 or combat >= 10.0 or rank() >= 5:
		return "STRATEGY PVP"
...
## FL-A/B/C/D/E/F/G/H/I/J/K/L/M/N: name the Strategy overlay fleet pip. Never DPS / yield / thrust.
