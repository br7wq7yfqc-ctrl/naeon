...
func _refresh() -> void:
		...
		for n in (SoftScanCache.get_pads() if SoftScanCache else tree.get_nodes_in_group("pad_bases")):
			if n is Node3D and n.has_method("get_faction"):
				...
				if st == "contested" or str(n.get_faction()) == "Contested":
					contested_near = true
				if n.has_method("ownership_state_label"):
					var own_l := str(n.ownership_state_label())
	...
	if tree:
		for stn in tree.get_nodes_in_group("player_orbital_stations"):
			...
			if stn.has_method("turret_hud_line"):
...
func _refresh_os_stack(pocket: bool, pad: Node) -> void:
	...
	if ally_line != "":
		body += "\n" + ally_line
	if pad != null and is_instance_valid(pad) and pad.has_method("ownership_state_label"):
