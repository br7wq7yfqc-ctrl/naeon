...
const FIELDS := [
	...
	"econ", "econ_rate", "econ_grot", "econ_rank", "know_rank", "contrib_rank",
	"energy", "energy_max",
...
static func snapshot(ship: Node = null, player: Node = null, pad: Node = null) -> Dictionary:
	...
	if GameManager:
		...
		if grot:
			...
			snap["econ_rank"] = -1
			if GameManager.has_method("contribution_rank"):
				snap["contrib_rank"] = int(GameManager.contribution_rank())
			else:
				snap["contrib_rank"] = 0
...
static func _fill_fleet(snap: Dictionary, ship: Node, player: Node) -> void:
	## FL-A/B/C/D/E/F/G/H/I/J/K/L/M/N: SoftKnowledge count only. Cap 15 (player + visitor + thirteen SoftNet).
		...
		for t in tree.get_nodes_in_group("pad_traffic"):
			...
			if t.has_method("fleet_guest_b"):
				...
				if gb != null and is_instance_valid(gb):
					n += 1
			if t.has_method("fleet_guest_c"):
				var gc: Node = t.fleet_guest_c()
				if gc != null and is_instance_valid(gc):
					n += 1
			if t.has_method("fleet_guest_d"):
			...
			if t.has_method("fleet_guest_e"):
				...
				if ge != null and is_instance_valid(ge):
					n += 1
			if t.has_method("fleet_guest_f"):
				var gf: Node = t.fleet_guest_f()
				if gf != null and is_instance_valid(gf):
					n += 1
			if t.has_method("fleet_guest_g"):
				var gg: Node = t.fleet_guest_g()
				if gg != null and is_instance_valid(gg):
					n += 1
			if t.has_method("fleet_guest_h"):
			...
			if t.has_method("fleet_guest_i"):
				...
				if gi != null and is_instance_valid(gi):
					n += 1
			if t.has_method("fleet_guest_j"):
				var gj: Node = t.fleet_guest_j()
				if gj != null and is_instance_valid(gj):
					n += 1
			if t.has_method("fleet_guest_k"):
				var gk: Node = t.fleet_guest_k()
				if gk != null and is_instance_valid(gk):
					n += 1
			if t.has_method("fleet_guest_l"):
				var gl: Node = t.fleet_guest_l()
				if gl != null and is_instance_valid(gl):
					n += 1
			if t.has_method("fleet_guest_m"):
				var gm: Node = t.fleet_guest_m()
				if gm != null and is_instance_valid(gm):
					n += 1
			if t.has_method("fleet_guest_n"):
				var gn: Node = t.fleet_guest_n()
				if gn != null and is_instance_valid(gn):
					n += 1
			if n > 1:
				break
	snap["fleet"] = mini(n, 15)
	snap["fleet_max"] = 15
...
static func stack_text(snap: Dictionary) -> String:
	...
	if grot:
		...
		econ_s += "  ·  %s" % _SoftK.contribution_rank_label(int(snap.get("contrib_rank", 0)))
	econ_s += "  ·  %s %s" % [
