...
- Basic Strategy / Space / TPS PvP — **PV-A built** (TPS on occupied unnamed pad) + **PV-B built** (seated hull on OpenSpace) + **PV-C built** (ST-A Strategy overlay, key B): same host-authority rival `CombatDummy` / SoftNet visual; Pulse 11 both ways; win = rival HP → 0; no permadeath; Infection cap 5; Knowledge labels only (`PVP` / `STRATEGY PVP`); TPS PV-A and hull PV-B stay; click/select still ≠ combat; G5 closed; no `SITE_*`
...
- Persistent colonies / ships — **PC-A built:** SoftSession snapshot of occupied unnamed-pad stubs (ST-H turret / ST-I storage / ST-J hangar) and/or existing `PlayerOrbitalStation` cluster modules (ST-E dock+habitat, ST-G factory, ST-K hangar, ST-L turret, ST-M storage) plus soft ship hull/form/faction; restore via existing `BaseBuilder.place_pad_*` / `place_orbital_*`; SoftKnowledge / HUD `COLONY` / `SHIP` / `SAVED` only; host authority; leave/rejoin same legal footprint; no DPS/yield; no `SITE_*`; no second world; Infection cap 5; ST-M / PV-C / AR-Z stay; `ORBITAL_STATIONS` stays false
