...
- Persistent colonies / ships — **PC-A+PC-B built:** SoftSession `user://soft_session.json` remembers pad player modules (habitat/turret/storage/hangar), orbital extras (ST-K/L/M hangar/turret/storage) and ship faction/module kinds (**PC-A**); plus ONE crate contents (amount/slug) for PadStorage (ST-I pad and/or ST-M orbital) and/or ship `CargoHold` (**PC-B**); restore via `BaseBuilder` (modules) and existing PadStorage / CargoHold APIs (crate) on relaunch; SoftKnowledge / HUD `COLONY` / `SHIP` / `CRATE` / `CARGO` / `PERSIST` only; host authority; does not steal the ST-A `player_module` slot; never Pulse / kit / P2W. Hangar insurance / full colony sim later. Not Voice. Not aiNEX.
