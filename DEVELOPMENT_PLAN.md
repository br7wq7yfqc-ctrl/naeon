...
- Multi-crew (2–4 игрока, роли) — **MC-A + MC-B + MC-C built:** one extra `CrewSeat` on the player hull pocket; station role `gunner` is a SoftKnowledge / HUD label only; third seat `engineer` (HUD CREW n/3; local-first, host authority; Pulse / Hack / thrust / yield stay)
