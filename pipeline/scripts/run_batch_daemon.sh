...
LOG=logs/batch_daemon.log
mkdir -p logs
exec >>"$LOG" 2>&1
echo "==== DAEMON START $(date -u) ===="

python3 -u - << 'PY'
import json, os, sys, time, subprocess
from pathlib import Path
ROOT = Path.cwd()
items = json.loads((ROOT/"pipeline/briefs/vertical_slice_batch.json").read_text())
MIN = 150
...
for item in items:
    ...
    if not glb.exists():
        ...
        print("REUSE inbox", glb, flush=True)
    ...
    if r.returncode!=0 or not has_asset(cat, name):
        # fallback check props misfile
        if (ROOT/"assets/props"/name/"manifest.json").exists():
            ...
            print("RELOCATED", name, "to", cat, flush=True)
        else:
            print("PROC FAIL", name, flush=True); failed.append(name); continue
    adir=ROOT/"assets"/cat/name
    subprocess.run(["rclone","copy",str(adir),f"neon:neon/dev/{cat}/{name}"], cwd=str(ROOT))
    done.append(name)
    print(f"OK {name} bal={bal()}", flush=True)

summary={"done":done,"failed":failed,"balance":bal(),"ts":time.time()}
print("SUMMARY", json.dumps(summary), flush=True)
(ROOT/"logs"/"batch_summary.json").write_text(json.dumps(summary, indent=2))
PY
echo "==== DAEMON END $(date -u) ===="
