...
mkdir -p logs
exec >>logs/batch4_daemon.log 2>&1
echo "==== BATCH4 START $(date -u) ===="
python3 -u - << 'PY'
import json, os, sys, subprocess, shutil
from pathlib import Path
ROOT = Path.cwd()
items = json.loads((ROOT/"pipeline/briefs/vertical_slice_batch4.json").read_text())
MIN = 150
def bal():
...
for item in items:
    ...
    if not glb.exists():
        ...
        if r.returncode!=0 or not glb.exists():
            failed.append(name); print("GEN FAIL", name, flush=True); continue
    ...
    if not has_asset(cat, name):
        if (ROOT/"assets/props"/name/"manifest.json").exists():
            ...
            failed.append(name); print("PROC FAIL", name, flush=True); continue
...
(ROOT/"logs"/"batch4_summary.json").write_text(json.dumps({"done":done,"failed":failed,"balance":bal()}, indent=2))
PY
echo "==== BATCH4 END $(date -u) ===="
